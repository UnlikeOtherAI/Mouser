...
use mouser_engine::{CaptureDecision, Edge, EdgeLayout, EngineCore, RuntimeHandle};
use mouser_net::{DeviceIdentity, InteractiveEndpoint, PinPolicy};

#[derive(Debug, PartialEq, Eq)]
enum Recorded {
    Move { display_id: u32, x: i32, y: i32 },
...
impl InputInjection for RecordingInjector {
    fn move_cursor(&self, display_id: u32, x: i32, y: i32) -> PlatformResult<()> {
        let _ = self.tx.send(Recorded::Move { display_id, x, y });
        Ok(())
    }
...
async fn source_drives_target_over_quic() {
    ...
    let source_capture = Arc::new(SpyCapture::default());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn left_edge_source_seeds_target_right_edge_over_quic() {
    let source_id = DeviceIdentity::generate();
    let target_id = DeviceIdentity::generate();
    let source_device = source_id.device_id();
    let target_device = target_id.device_id();
    ...
    let source_rt = RuntimeHandle::start(
        EngineCore::new_source(
            source_device,
            target_device,
            EdgeLayout::with_edge(100, 100, 1440, 100, Edge::Left),
        ),
        Arc::new(source_conn),
        Arc::new(NoopInjector),
        spy.clone(),
    ...
    let decision = source_rt.feed_local(LocalInputEvent::CursorMoved {
        display_id: 0,
        x: 1,
        y: 40,
        dx: -6,
    ...
    assert_eq!(
        ...
        "the crossing event is swallowed locally"
    );
    await_mode(&spy, CaptureMode::ActiveForward).await;
    assert!(!source_rt.is_owner(), "source handed input to the target");

    let mut got_right_edge_move = false;
    for _ in 0..40 {
        source_rt.feed_local(LocalInputEvent::CursorMoved {
            display_id: 0,
            x: 1,
        ...
        match tokio::time::timeout(Duration::from_millis(200), rec_rx.recv()).await {
            Ok(Some(Recorded::Move { display_id, x, y })) => {
                assert_eq!(display_id, 0);
                assert_eq!(x, 1439, "left-edge entry lands on target right edge");
                assert_eq!(y, 40);
                got_right_edge_move = true;
                break;
            }
            Ok(Some(_)) => {}
            Ok(None) => break,
    ...
    assert!(
        got_right_edge_move,
        "target received a left-edge cursor move at the peer right edge"
    );
    ...
    assert!(got_key, "target injected input after the left-edge seed");
