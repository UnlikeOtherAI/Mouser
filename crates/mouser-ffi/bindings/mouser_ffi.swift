...
#if canImport(mouser_ffiFFI)
import mouser_ffiFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_mouser_ffi_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_mouser_ffi_rustbuffer_free(self, $0) }
    }
}
...
private func makeRustCall<T, E: Swift.Error>(
    ...
    uniffiEnsureMouserFfiInitialized()
...
public protocol MobileClientProtocol: AnyObject, Sendable {
     ...
     */
    func sendPointerMoved(displayId: UInt32, x: Int32, y: Int32) 
    
    /**
     * Report a scroll delta in logical pixels.
     */
    func sendScroll(dx: Int32, dy: Int32) 
...
open class MobileClient: MobileClientProtocol, @unchecked Sendable {
    ...
    public func uniffiCloneHandle() -> UInt64 {
        return try! rustCall { uniffi_mouser_ffi_fn_clone_mobileclient(self.handle, $0) }
    }
    /**
...
public convenience init() {
    let handle =
        try! rustCall() {
    uniffi_mouser_ffi_fn_constructor_mobileclient_new($0
    )
}
    ...
    deinit {
        ...
        try! rustCall { uniffi_mouser_ffi_fn_free_mobileclient(handle, $0) }
    }
...
open func deviceId() -> String  {
    ...
    uniffi_mouser_ffi_fn_method_mobileclient_device_id(
...
open func disconnect()  {try! rustCall() {
    uniffi_mouser_ffi_fn_method_mobileclient_disconnect(
            self.uniffiCloneHandle(),$0
    )
}
}
...
open func isConnected() -> Bool  {
    ...
    uniffi_mouser_ffi_fn_method_mobileclient_is_connected(
            self.uniffiCloneHandle(),$0
    )
})
}
     ...
     * (0=left, 1=right, 2=middle, 3=back, 4=forward).
     */
open func sendButton(button: UInt8, down: Bool)  {try! rustCall() {
    uniffi_mouser_ffi_fn_method_mobileclient_send_button(
            self.uniffiCloneHandle(),
        FfiConverterUInt8.lower(button),
     ...
     * active modifier bitmask (`down` presses).
     */
open func sendKey(usage: UInt16, down: Bool, mods: UInt16)  {try! rustCall() {
    uniffi_mouser_ffi_fn_method_mobileclient_send_key(
            self.uniffiCloneHandle(),
        FfiConverterUInt16.lower(usage),
        FfiConverterBool.lower(down),
        FfiConverterUInt16.lower(mods),$0
    )
}
}
     ...
     * absolute result to its real display.
     */
open func sendPointerMoved(displayId: UInt32, x: Int32, y: Int32)  {try! rustCall() {
    uniffi_mouser_ffi_fn_method_mobileclient_send_pointer_moved(
            self.uniffiCloneHandle(),
        FfiConverterUInt32.lower(displayId),
        FfiConverterInt32.lower(x),
        FfiConverterInt32.lower(y),$0
    )
}
}
     ...
     * Report a scroll delta in logical pixels.
     */
open func sendScroll(dx: Int32, dy: Int32)  {try! rustCall() {
    uniffi_mouser_ffi_fn_method_mobileclient_send_scroll(
            self.uniffiCloneHandle(),
        FfiConverterInt32.lower(dx),
        FfiConverterInt32.lower(dy),$0
    )
}
}
...
private let initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_mouser_ffi_uniffi_contract_version()
    ...
    if (uniffi_mouser_ffi_checksum_method_mobileclient_send_button() != 37969) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mouser_ffi_checksum_method_mobileclient_send_key() != 56450) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mouser_ffi_checksum_method_mobileclient_send_pointer_moved() != 37151) {
        return InitializationResult.apiChecksumMismatch
    }
...
public func uniffiEnsureMouserFfiInitialized() {
