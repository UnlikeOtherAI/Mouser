...
/// One in-flight inbound pull, keyed by `(origin, hash)`.
struct Pending {
    origin: DeviceId,
    reasm: Reassembly,
}
...
impl ClipboardEngine {
    ...
    /// disabled format yields nothing. A small payload (≤ [`CONTROL_TEXT_CAP`]) is one
    /// message (`offset = 0`, `last = true`) for the control stream; a larger one is
    /// split into ≤ [`MAX_DATA_CHUNK`] (1 MiB) ordered chunks for the bulk plane.
    /// Returns an empty vec when the format is disabled or `source` no longer holds the
    /// content (the clipboard moved on).
...
/// A payload at or below [`CONTROL_TEXT_CAP`] becomes a single control-stream message;
/// a larger one is split into ≤ [`MAX_DATA_CHUNK`] (1 MiB) bulk chunks. An empty
/// payload still emits one terminal chunk (`offset = 0`, `last = true`).
fn chunk_data(hash: &Hash, format: ClipFormat, bytes: &[u8]) -> Vec<ClipboardData> {
    if bytes.len() <= CONTROL_TEXT_CAP {
